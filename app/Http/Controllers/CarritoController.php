...
class CarritoController extends Controller
{
    ...
    public function agregar(Request $request)
    {
        ...
        // Cantidad que ya tiene en el carrito (0 si es nuevo)
        $cantidadEnCarrito = $item ? $item->cantidad : 0;

        // Verificar que la suma total no supere el stock real
        if ($producto->stock < ($cantidadEnCarrito + $request->cantidad)) {
            return back()->with('error', "Stock insuficiente. Tenés {$cantidadEnCarrito} en el carrito y solo hay {$producto->stock} disponibles.");
        }
    ...
    public function confirmar(Request $request)
    {
        ...
        if ($request->metodo_pago === 'tarjeta') {
            $reglas['tarjeta_numero']      = ['required', 'digits_between:13,19'];
            ...
            $reglas['tarjeta_cvv']         = ['required', 'digits_between:3,4'];
        ...
        DB::transaction(function () use ($carrito, $request) {
            ...
            if ($request->metodo_pago === 'tarjeta') {
                ...
                $datos['tarjeta_cuotas']      = $request->tarjeta_cuotas;
